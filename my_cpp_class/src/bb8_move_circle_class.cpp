...
#include <my_cpp_class/custom_message.h>
#include <ros/ros.h>
...
public:
  ...
  void move_in_circle(int duration) {
    ros::Duration rate = ros::Duration(duration);

    ROS_INFO("Duration to move: %d", duration);
  ...
  bool my_callback(my_cpp_class::custom_message::Request &req,
                   my_cpp_class::custom_message::Response &res) {
